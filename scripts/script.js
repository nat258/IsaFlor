...
const regionesComunas = {'Region Metropolitana': ['Santiago','Puente Alto','Maipu',
        'La Florida'
    ],
...
const productosBase = [
    {
        ...
        precio: 19990,
        imagen: '../../imagenes/rosas.webp'
    },

    {
        codigo: 'TUL02',
        nombre: 'Tulipan premium',
        precio: 14990,
        imagen: '../../imagenes/tulipanes.webp'
    },

    {
        ...
        precio: 24990,
        imagen: '../../imagenes/orquideas.webp'
    },

    {
        codigo: 'CART',
        nombre: 'Tarjeta dedicatoria',
        precio: 500,
        imagen: '../../imagenes/tarjeta.webp'
    }
];
...
function correoValido(correo) {
    ...
    return formato.test(valor) &&
        DOMINIOS_PERMITIDOS.some((dominio) => valor.endsWith(dominio));
}
...
function inicializarLogin() {
    ...
    form.addEventListener('submit', (event) => {
        ...
        if (!correo) {
            ...
            mostrarError(
                'login-correo',
                'Ingresa un correo valido con dominio permitido.'
            );
        ...
        mensaje.textContent =
            'Inicio de sesion validado correctamente.';

        const botonPresionado = event.submitter;

        if (
            botonPresionado &&
            botonPresionado.id === 'btn-admin'
        ) {
            window.location.href =
                'VistaAdmin/inicioAdmin.html';
        }
    });
}
    ...
    form.addEventListener('submit', (event) => {
        ...
        if (correo) {
            if (
                ...
                mostrarError(
                    'contacto-correo',
                    'El correo debe tener un dominio permitido.'
                );
...
function poblarRegionesYComunas(form) {
    ...
    regionSelect.addEventListener('change', () => {
        const comunas =
            regionesComunas[regionSelect.value] || [];
...
const usuariosPrecargados = [
...

    {
        ...
        region: 'Biobio',
        comuna: 'Concepcion',
        direccion: 'Calle Los Jardines 321'
    }
];
...
function obtenerUsuarios() {
    ...
    if (!guardados) {
        localStorage.setItem(
            STORAGE_USUARIOS,
            JSON.stringify(usuariosPrecargados)
        );
...
function renderizarUsuarios() {
    ...
    usuarios.forEach((usuario) => {
        ...
        fila.innerHTML =
            ...
            '<td>' +
                '<button type="button" class="boton boton-secundario" data-editar-usuario="' + usuario.run + '">' +
                    'Editar' +
                '</button> ' +
                '<button type="button" class="boton" data-eliminar-usuario="' + usuario.run + '">' +
                    'Eliminar' +
                '</button>' +
            '</td>';
    ...
    contenedor.querySelectorAll(
        ...
        boton.addEventListener('click', () => {
            const run =
                boton.getAttribute('data-eliminar-usuario');
    ...
    contenedor.querySelectorAll(
        ...
        boton.addEventListener('click', () => {
            const run =
                boton.getAttribute('data-editar-usuario');
...
function editarUsuario(run) {
    ...
    localStorage.setItem(
        'isaflor-usuario-editar',
        JSON.stringify(usuario)
    );

    window.location.href =
        'nuevoUsuario.html?editar=' +
...
function validarFormularioUsuario(form, mensajeId) {
    ...
    form.addEventListener('submit', (event) => {
        ...
        const fechaNacimiento =
            form.querySelector('#fechaNacimiento')
                ?.value || '';

        const tipoUsuario =
            form.querySelector('#tipoUsuario')
                .value;
        ...
        if (!run) {
            ...
            mostrarError(
                'run',
                'Ingresa un RUN valido sin puntos ni guion.'
            );
        ...
        if (!direccion) {
            ...
            mostrarError(
                'direccion',
                'La direccion no puede superar 300 caracteres.'
            );
    ...
    const formularios = [
        {
            ...
            mensajeId: 'mensaje-registro-cliente'
        },

        {
            id: 'form-nuevo-usuario',
            mensajeId: 'mensaje-nuevo-usuario'
        },

        {
            id: 'form-registro-admin',
            mensajeId: 'mensaje-registro-admin'
        }
    ];
...
function inicializarProducto() {
    ...
    form.addEventListener('submit', (event) => {
        ...
        if (
            ...
            mostrarError(
                'stock-critico',
                'El stock critico es obligatorio.'
            );
            ...
            mostrarError(
                'stock-critico',
                'El stock critico debe ser un entero mayor o igual a 0.'
            );
        ...
        document.getElementById(
            ...
            valido
                ? 'Producto validado y guardado correctamente.'
                : '';
...
function renderizarCarrito() {
    ...
    contenedor.querySelectorAll(
        ...
        input.addEventListener('change', () => {
            ...
            actualizarCantidad(
                input.getAttribute('data-cantidad'),
                cantidad
            );
        });
    });
    ...
    contenedor.querySelectorAll(
        ...
        boton.addEventListener('click', () => {
            ...
            eliminarDelCarrito(
                boton.getAttribute('data-eliminar')
            );
        });
    });
}
